...
from llm.model_selection import (
    ...
    fetch_installed_ollama_models,
    replace_config_compatible,
...
def _fetch_installed_ollama_models(config: AppConfig) -> list[dict]:
    return fetch_installed_ollama_models(config)
...
def _with_available_models(config: AppConfig) -> AppConfig:
    return with_available_models(
        config,
        fetch_installed_models=_fetch_installed_ollama_models,
    )


def _replace_config_compatible(config: AppConfig, **changes: object) -> AppConfig:
    return replace_config_compatible(config, **changes)


def _configured_model_candidates(config: object) -> tuple[str, ...]:
    return configured_model_candidates(config)
...
def _allowed_model_candidates(
    ...
    *,
    ...
    return allowed_model_candidates(
        config,
...
def _select_router_model(
    *,
    ...
    return select_router_model(
        preferred_router=preferred_router,
        installed_names=installed_names,
        primary_model=primary_model,
    )
