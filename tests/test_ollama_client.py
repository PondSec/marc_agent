...
def test_ollama_client_extends_small_model_startup_budget_for_long_same_model_followups(monkeypatch, tmp_path):
    ...
    result = client.generate(
        "Continue the same repair with the full runtime evidence.",
        timeout=90,
        total_timeout=420,
        retries=0,
        progress_callback=progress_events.append,
    ...
    assert result == "ready"
    assert captured == [280]
    assert progress_events[0]["startup_timeout"] == 280
    assert progress_events[0]["inactivity_timeout"] == 90
    assert progress_events[0]["total_timeout"] == 420
...
def test_ollama_client_marks_pre_chunk_startup_timeout_retryable(monkeypatch, tmp_path):
    config = AppConfig(
        workspace_root=str(tmp_path),
        model_name="qwen2.5-coder:14b",
        llm_timeout=20,
