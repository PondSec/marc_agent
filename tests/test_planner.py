...
def test_compact_repair_retry_prompt_surfaces_file_local_requirements(tmp_path):
    pkg = tmp_path / "texttools"
    pkg.mkdir()
    normalize_current = (
    ...
    (tests_dir / "test_normalize.py").write_text(
        ...
        "from texttools import normalize_words, normalize_words_keep_case\n\n"
        "class TestNormalize(unittest.TestCase):\n"
        "    def test_default_normalization_lowercases_words(self):\n"
    ...
    session = SessionState(
        task="Add keep_case support and repair the failing normalization behavior.",
        workspace_root=str(tmp_path),
        workspace_snapshot=snapshot,
    )
    payload = route_payload(
    ...
    repair_context = ValidationFailureEvidence(
        ...
        repair_brief=RepairBrief(
            failure_type="assertion_mismatch",
            failure_signature="runtime:assertion_mismatch:retry-file-local",
    ...
    assert "Repair-scoped requirements:" in prompt
    assert "normalize_words_keep_case" in prompt
    assert "tests/test_normalize.py" in prompt


def test_full_repair_retry_prompt_surfaces_file_local_requirements(tmp_path):
    pkg = tmp_path / "texttools"
    pkg.mkdir()
    normalize_current = (
        "def normalize_words(text: str) -> str:\n"
    ...
    repair_context = ValidationFailureEvidence(
        ...
        repair_brief=RepairBrief(
            failure_type="assertion_mismatch",
            failure_signature="runtime:assertion_mismatch:retry-file-local-full",
            primary_target="texttools/normalize.py",
            locked_target="texttools/normalize.py",
            expected_semantics=["Validation should produce: Hello WORLD"],
            observed_semantics=["Validation currently produces: Hello WORLD!"],
...
def test_full_repair_retry_prompt_trims_noise_after_hash_unchanged_noop(tmp_path):
    pkg = tmp_path / "texttools"
    pkg.mkdir()
    normalize_current = (
        "def normalize_words(text: str, keep_case: bool = False) -> str:\n"
    ...
    session.diagnostics.append(
        DiagnosticRecord(
            source="pre_write_update_review",
            category="preservation_risk",
            summary="An earlier README proposal added unrequested sections.",
            tool_name="internal:proposed_update_review",
            file_hints=["README.md"],
            excerpt="The proposed markdown update adds unrequested documentation sections.",
        )
    )
    session.validation_runs.append(
        ValidationRunRecord(
    ...
    repair_context = ValidationFailureEvidence(
        ...
        summary="Validation command exited with 1.",
        excerpt=(
            "FAIL: test_cli_supports_keep_case_flag (tests.test_normalize.TestNormalize.test_cli_supports_keep_case_flag)\n"
            "AssertionError: 'Hello WORLD!' != 'Hello WORLD'\n"
            "FAIL: test_default_normalization_lowercases_words (tests.test_normalize.TestNormalize.test_default_normalization_lowercases_words)\n"
            "AssertionError: 'hello world!' != 'hello world'\n"
        ),
        failure_summary="texttools/normalize.py still produces the wrong behavior across multiple validation assertions.",
        file_hints=["texttools/normalize.py", "normalize_cli.py", "tests/test_normalize.py"],
        repair_requirements=["Change texttools/normalize.py so the keep-case and default outputs both drop the observed-only punctuation."],
        repair_brief=RepairBrief(
            failure_type="assertion_mismatch",
            failure_signature="runtime:assertion_mismatch:retry-file-local-noop",
            primary_target="texttools/normalize.py",
            locked_target="texttools/normalize.py",
            expected_semantics=[
            ...
            implicated_symbols=["normalize_words", "normalize_words_keep_case"],
            implicated_region_hint="texttools/normalize.py",
            repair_constraints=["Stay on the locked normalization target and remove the observed-only punctuation."],
    ...
    review_feedback = ProposedUpdateReview(
        ...
        confidence=0.93,
        blocking_issues=[
            "The proposed repair does not make a productive change to texttools/normalize.py for runtime:assertion_mismatch:retry-file-local-noop (file hash unchanged). The locked repair target is still texttools/normalize.py."
        ],
        preservation_risks=[],
        repair_hints=[
            "Rewrite at least one implicated output line in texttools/normalize.py; leaving this excerpt unchanged will preserve the failing behavior:\n2:     if keep_case:\n3:         return ' '.join(text.replace(',', ' ').split())",
            "The current implementation in texttools/normalize.py never handles the observed-only literal '!' explicitly. Change the output construction or normalization so that literal no longer survives in the returned value.",
        ],
    )
    ...
    assert "Concrete behavior examples:" in prompt
    assert "Diagnostic context:" not in prompt
    assert "Self-review feedback on the previous proposal:" not in prompt
    assert "__REPAIR_BLOCKED__" in prompt
    assert "Hello WORLD!' -> 'Hello WORLD'" in prompt
    assert len(prompt) < 9000
...
def test_planner_runtime_repair_pivots_to_inferred_main_entrypoint_after_library_fix(tmp_path):
    pkg = tmp_path / "texttools"
    pkg.mkdir()
    (pkg / "__init__.py").write_text(
        "from .normalize import normalize_words, normalize_words_keep_case\n",
    ...
    (pkg / "normalize.py").write_text(
        ...
        "    return [part.lower() for part in parts]\n\n"
        "def normalize_words_keep_case(text: str) -> str:\n"
        "    return ' '.join(normalize_words(text, keep_case=True))\n",
        encoding="utf-8",
    )
    (tmp_path / "normalize_cli.py").write_text(
        "from texttools import normalize_words\n\n"
        "def main(argv=None):\n"
        "    keep_case = bool(argv and '--keep-case' in argv)\n"
        "    return normalize_words('Hello, WORLD!', keep_case=keep_case)\n",
        encoding="utf-8",
    )
    (tmp_path / "README.md").write_text("# texttools\n", encoding="utf-8")
    tests_dir = tmp_path / "tests"
    ...
    snapshot = RepoMemoryStore(config, WorkspaceManager(tmp_path)).build_snapshot("cli unittest")
    ...
    planner = Planner(llm, "")
    session = SessionState(
        task="Ergänze keep_case in texttools und der CLI, sodass python -m unittest tests.test_normalize grün wird.",
        workspace_root=str(tmp_path),
        workspace_snapshot=snapshot,
        validation_status="failed",
        edit_generation=3,
    ...
    session.validation_runs.append(
        ValidationRunRecord(
            ...
            excerpt=(
                ...
                "----------------------------------------------------------------------\n"
                f'  File "{tests_dir / "test_normalize.py"}", line 17, in test_cli_supports_keep_case_flag\n'
                "    self.assertEqual(output.getvalue().strip(), 'Hello WORLD')\n"
...
def test_full_repair_retry_prompt_surfaces_stdout_capture_and_direct_main_argv_contract(tmp_path):
    ...
    assert "hardcoding sample output" in prompt
...
def test_compact_repair_prompt_prioritizes_test_line_hints_for_stdout_contract(tmp_path):
    ...
    repair_context = ValidationFailureEvidence(
        ...
        repair_brief=RepairBrief(
            failure_type="assertion_mismatch",
            failure_signature="runtime:assertion_mismatch:normalize-cli-test-line-hints",
            primary_target="normalize_cli.py",
            locked_target="normalize_cli.py",
            expected_semantics=["Validation should produce: Hello WORLD"],
            observed_semantics=["Validation currently produces: ''"],
...
def test_should_skip_model_backed_repair_review_disables_skip_after_prior_failed_mutation_on_same_target(tmp_path):
    ...
    session.active_repair_context = ValidationFailureEvidence(
        ...
        action_hints=[],
        repair_requirements=["Change texttools/normalize.py so the keep-case output drops the observed-only punctuation."],
        evidence_signature="sig-normalize-behavior-mutation-history",
        repair_brief=RepairBrief(
            failure_type="assertion_mismatch",
            failure_signature="runtime:assertion_mismatch:normalize-behavior-mutation-history",
            primary_target="texttools/normalize.py",
            locked_target="texttools/normalize.py",
            expected_semantics=["Validation should produce: Hello WORLD"],
            observed_semantics=["Validation currently produces: Hello WORLD!"],
    ...
    session.repair_history.append(
        RepairAttemptRecord(
            ...
            strategy="validation_targeted",
            result="mutation_planned",
            reason="Prepared a local normalization patch, but validation still failed afterwards.",
            evidence_signature="sig-normalize-behavior-mutation-history",
            failure_signature="runtime:assertion_mismatch:normalize-behavior-mutation-history",
        )
    )
...
def test_pre_write_update_review_keeps_scope_rejection_when_stdout_contract_is_unaddressed(
    tmp_path,
    monkeypatch,
):
    planner = Planner(ScriptedLLM(), "")
    ...
    session.active_repair_context = ValidationFailureEvidence(
        ...
        repair_brief=RepairBrief(
            failure_type="assertion_mismatch",
            failure_signature="runtime:assertion_mismatch:stdout-contract-scope-guard",
            primary_target="normalize_cli.py",
            locked_target="normalize_cli.py",
            expected_semantics=["Validation should produce: Hello WORLD"],
            observed_semantics=["Validation currently produces: ''"],
...
def test_pre_write_update_review_rejects_stdout_only_fix_when_direct_main_option_contract_is_unchanged(
    tmp_path,
):
    planner = Planner(ScriptedLLM(), "")
    tests_dir = tmp_path / "tests"
    ...
    session.active_repair_context = ValidationFailureEvidence(
        ...
        repair_brief=RepairBrief(
            failure_type="assertion_mismatch",
            failure_signature="runtime:assertion_mismatch:direct-main-option-contract",
            primary_target="normalize_cli.py",
            locked_target="normalize_cli.py",
            expected_semantics=["Validation should produce: Hello WORLD"],
            observed_semantics=["Validation currently produces: Hello World."],
...
def test_pre_write_update_review_rejects_direct_main_option_fix_with_inexact_token_spellings(
    ...
    session.active_repair_context = ValidationFailureEvidence(
        ...
        repair_brief=RepairBrief(
            failure_type="assertion_mismatch",
            failure_signature="runtime:assertion_mismatch:direct-main-option-exact-token",
            primary_target="normalize_cli.py",
            locked_target="normalize_cli.py",
            expected_semantics=["Validation should produce: Hello WORLD"],
            observed_semantics=["Validation currently produces: hello world."],
...
def test_pre_write_update_review_allows_direct_main_option_contract_fix_when_argv_lines_change(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: ProposedUpdateReview(
            safe_to_write=True,
            summary="The proposal stays focused on the direct runtime contract.",
            confidence=0.89,
            blocking_issues=[],
            preservation_risks=[],
            repair_hints=[],
