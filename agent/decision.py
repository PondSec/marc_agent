...
class ExecutionDecisionPolicy:
        ...
        *,
        ...
        if self._should_clarify(
            ...
            route = RouterOutput(
                ...
                requested_outcome="Clarify the missing target, scope, or risk before taking action.",
                action_plan=[
                    {
                        "step": 1,
        ...
        route = RouterOutput(
            ...
            requested_outcome=self._requested_outcome(understanding, task_state=task_state),
    ...
    def _requested_outcome(
        ...
        *,
        ...
        if strategy == "rollback_correction":
            return "Correct the active scope or prior change with the smallest necessary update, then verify the new boundary."
        if strategy == "hardening":
            return "Harden the targeted behavior with minimal scope and verify that intended behavior still works."
        if understanding.intent_category == "debug":
            return "Diagnose the issue against current evidence, then apply the smallest safe fix."
        if understanding.intent_category == "refactor":
            return "Improve structure and maintainability while preserving behavior."
        if understanding.intent_category == "test":
            return "Verify the current implementation and surface the relevant failures clearly."
    ...
    def _action_plan(
        ...
        *,
        ...
        if intent == RouteIntent.PLAN:
            actions = [
                (RouteActionName.PLAN_WORK, "The user primarily wants a concrete plan before execution."),
            ]
        elif intent == RouteIntent.SEARCH:
            actions = [
                (RouteActionName.SEARCH_WORKSPACE, "The target should be located semantically in the workspace first."),
                (RouteActionName.READ_RELEVANT_FILES, "Inspect the strongest search hits before answering."),
                (RouteActionName.SUMMARIZE_RESULT, "Return the located implementation or findings."),
            ]
        elif intent == RouteIntent.INSPECT:
            actions = [
                (RouteActionName.INSPECT_WORKSPACE, "Collect focused repository context before answering."),
                (RouteActionName.READ_RELEVANT_FILES, "Inspect the most relevant artifacts before summarizing."),
                (RouteActionName.SUMMARIZE_RESULT, "Return a concise explanation of the findings."),
            ]
        elif intent == RouteIntent.EXPLAIN:
            if target_paths:
                actions = [
                    (RouteActionName.READ_RELEVANT_FILES, "Read the referenced artifacts before explaining them."),
                    (RouteActionName.SUMMARIZE_RESULT, "Explain the result in user-facing language."),
                ]
            else:
                actions = [
                    (RouteActionName.RESPOND_DIRECTLY, "The request can be answered directly from the interpreted goal."),
                ]
        elif intent == RouteIntent.CREATE:
            if snapshot is not None and snapshot.file_count > 0:
                actions.append(
                    (RouteActionName.INSPECT_WORKSPACE, "Inspect existing project conventions before creating new code.")
                )
            actions.extend(
                [
                    (RouteActionName.CREATE_ARTIFACT, "Create the requested artifact in the smallest sensible scope."),
                    (
                        RouteActionName.RUN_VALIDATION,
                        self._validation_reason(task_state, fallback="Validate the new implementation against the project command plan."),
                    ),
                    (RouteActionName.SUMMARIZE_RESULT, "Report what was created and how it was verified."),
                ]
            )
                ...
                actions.append(
                    (
                        ...
                        self._pre_edit_reason(
                            strategy,
                            fallback="Inspect the active implementation before editing it.",
                        ),
                    )
                )
            else:
                actions.extend(
                    [
                        (
                            ...
                            "Find the most relevant implementation area before editing.",
                        ),
                        (
                            RouteActionName.READ_RELEVANT_FILES,
                            self._pre_edit_reason(
                                strategy,
                                fallback="Inspect the likely targets before changing them.",
                            ),
                        ),
                    ]
                )
            actions.extend(
                [
                    (
                        ...
                        self._update_reason(
                            strategy,
                            fallback="Apply the focused change implied by the interpreted goal.",
                        ),
                    ),
                    (
                        RouteActionName.RUN_VALIDATION,
                        self._validation_reason(task_state, fallback="Validate the updated behavior after the change."),
                    ),
                    (RouteActionName.SUMMARIZE_RESULT, "Report the change, assumptions, and verification result."),
                ]
            )
            ...
            if target_paths:
                actions.append(
                    (
                        RouteActionName.READ_RELEVANT_FILES,
                        "Inspect the active artifact before reproducing the failure or editing it.",
                    )
                )
            actions.extend(
                [
                    (
                        ...
                        "Translate the user report and available evidence into concrete diagnostics before fixing anything.",
                    ),
                ]
            )
            if understanding.intent_category != "test":
                actions.append(
                    (RouteActionName.UPDATE_ARTIFACT, "If diagnostics confirm the cause, apply the smallest safe fix.")
                )
            actions.extend(
                [
                    (
                        RouteActionName.RUN_VALIDATION,
                        self._validation_reason(task_state, fallback="Rerun the most relevant validation after diagnosis or repair."),
                    ),
                    (RouteActionName.SUMMARIZE_RESULT, "Report the diagnosis, change, and validation outcome."),
                ]
            )
        else:
            actions = [
                (RouteActionName.INSPECT_WORKSPACE, "Gather repository context before choosing a concrete next step."),
                (RouteActionName.SUMMARIZE_RESULT, "Return the findings and any remaining ambiguity."),
            ]
    ...
    def _pre_edit_reason(
        ...
        *,
        ...
        if strategy == "rollback_correction":
            return "Inspect the previously changed surface before narrowing or correcting it."
        if strategy == "refactor":
            return "Inspect the active implementation first so behavior stays stable during the refactor."
        if strategy == "hardening":
            return "Inspect the active implementation before tightening safety or reliability."
        return fallback
    ...
    def _update_reason(
        ...
        *,
        ...
        if strategy == "rollback_correction":
            return "Revert or narrow only the necessary part of the previous interpretation or change."
        if strategy == "refactor":
            return "Apply an incremental refactor while preserving behavior."
        if strategy == "hardening":
            return "Apply the highest-value hardening change without broad rewrites."
        return fallback
    ...
    def _validation_reason(
        ...
        *,
        ...
        if task_state is None or not task_state.verification_target:
            return fallback
        return f"Verify against the committed target: {task_state.verification_target}"
    ...
    def _clarification_reason(
        ...
        *,
        ...
        if understanding.risk_level == "high":
            return "The request carries high execution risk without enough target certainty."
        if intent == RouteIntent.CREATE and not self._can_default_create_safely(
            understanding,
            target_paths=target_paths,
            target_name=target_name,
        ):
            return "The request still lacks enough implementation detail to choose a sensible default artifact safely."
        if intent in {RouteIntent.UPDATE, RouteIntent.DEBUG} and not target_paths:
            return "The likely task type is clear, but the concrete target artifact is still too uncertain."
        if understanding.missing_info:
            return "Critical scope information is still missing for a safe next step."
        return "The task understanding is still too ambiguous for safe execution."
